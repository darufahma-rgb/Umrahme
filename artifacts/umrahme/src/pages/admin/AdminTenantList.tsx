  ...
  return (
    <AdminLayout>
      ...
      {toast && (
        ...
        >
          <svg className="flex-none" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div className="flex items-center justify-between mb-7">
        <div>
          <h1
            ...
            style={{ fontSize: '24px', color: '#111827', letterSpacing: '-0.02em' }}
          >
            Daftar Tenant
          </h1>
          <p className="font-mono text-[11px] mt-0.5" style={{ color: '#9ca3af', letterSpacing: '0.01em' }}>
            {loading ? '—' : `${tenants.length} tenant terdaftar`}
          </p>
        </div>
        <Link
          to="/admin/tenants/baru"
          className="inline-flex items-center gap-2 font-semibold text-[13px] px-4 py-2.5 rounded-xl transition-all duration-150 active:scale-[0.98]"
          style={{
            background: 'linear-gradient(135deg, #4338ca 0%, #4f46e5 100%)',
            color: '#ffffff',
            boxShadow: '0 2px 8px rgba(67,56,202,0.26), 0 1px 2px rgba(67,56,202,0.18)',
          }}
        >
      ...
      {loading ? (
        <div
          className="rounded-2xl py-16 text-center font-mono text-[12px]"
          style={{ background: '#ffffff', border: '1px solid rgba(0,0,0,0.06)', color: '#d1d5db' }}
        >
          Memuat data...
        </div>
      ) : tenants.length === 0 ? (
        <div
          ...
          style={{ background: '#ffffff', border: '1px solid rgba(0,0,0,0.06)', color: '#9ca3af' }}
        >
          Belum ada tenant. Tambah yang pertama!
        </div>
      ) : (
        <div
            ...
            <tbody>
              {tenants.map((t, i) => (
                ...
                >
                  ...
                  <td className="px-5 py-4">
                    <span className="text-sm font-semibold" style={{ color: '#111827' }}>{t.nama_travel}</span>
                  </td>
                  <td className="px-5 py-4 hidden sm:table-cell">
                    <div className="flex items-center gap-2.5">
                      <span
                        className="inline-block w-6 h-6 rounded-full flex-none"
                        style={{
                          background: t.primary_color,
                          boxShadow: `0 0 0 2px white, 0 0 0 3px ${t.primary_color}40`,
                        }}
                      />
                      <span className="font-mono text-[11px]" style={{ color: '#9ca3af', letterSpacing: '0.04em' }}>
                        {t.primary_color}
                      </span>
                    </div>
                  </td>
                  ...
                  <td className="px-5 py-4 text-right">
                    <Link
                      ...
                      className="inline-flex items-center gap-1.5 text-[12px] font-semibold px-3 py-1.5 rounded-lg transition-all duration-150"
                      style={{ color: '#4338ca' }}
